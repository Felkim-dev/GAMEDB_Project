...
if "error" in available_reports:
    ...
    # ==================== REPORTE 4: Detalles de Transacciones ====================
        ...
        if "error" not in characters and characters.get("characters"):
            ...
            pruebas=st.button("🔍Ver Perfil", use_container_width=True)
            if pruebas:
                ...
                if "error" in data:
                    ...
                    csv_buffer.write("=== INFORMACIÓN BÁSICA ===\n")
                    csv_buffer.write(f"CharacterID,{info['CharacterID']}\n")
                    csv_buffer.write(f"CharacterName,{info['Name']}\n")
                    csv_buffer.write(f"Level,{info['Level']}\n")
                    csv_buffer.write(f"Experience,{info['Experience']}\n")
                    csv_buffer.write(f"PlayerID,{info['PlayerID']}\n")
                    csv_buffer.write(f"PlayerName,{info['PlayerName']}\n")
                    csv_buffer.write(f"PlayerEmail,{info['PlayerEmail']}\n\n")

                    # Inventario
                    csv_buffer.write("=== INVENTARIO ===\n")
                    if profile["inventory"]:
                        inv_df = pd.DataFrame(profile["inventory"])
                        inv_df.to_csv(csv_buffer, index=False)
                    else:
                        csv_buffer.write("Sin items en el inventario\n")
                    csv_buffer.write("\n")

                    # Misiones
                    csv_buffer.write("=== MISIONES ===\n")
                    if profile["missions"]:
                        miss_df = pd.DataFrame(profile["missions"])
                        miss_df.to_csv(csv_buffer, index=False)
                    else:
                        csv_buffer.write("Sin misiones asignadas\n")
                    csv_buffer.write("\n")

                    # Transacciones dadas
                    csv_buffer.write("=== TRANSACCIONES DADAS ===\n")
                    if profile["transactions_given"]:
                        given_df = pd.DataFrame(profile["transactions_given"])
                        given_df.to_csv(csv_buffer, index=False)
                    else:
                        csv_buffer.write("Sin transacciones salientes\n")
                    csv_buffer.write("\n")

                    # Transacciones recibidas
                    csv_buffer.write("=== TRANSACCIONES RECIBIDAS ===\n")
                    if profile["transactions_received"]:
                        rec_df = pd.DataFrame(profile["transactions_received"])
                        rec_df.to_csv(csv_buffer, index=False)
                    else:
                        csv_buffer.write("Sin transacciones entrantes\n")

                    # Botón de descarga
                    st.download_button(
                        label="📥 Descargar Perfil Completo (CSV)",
                        data=csv_buffer.getvalue(),
                        file_name=f"perfil_{info['Name']}_{char_id}.csv",
                        mime="text/csv",
